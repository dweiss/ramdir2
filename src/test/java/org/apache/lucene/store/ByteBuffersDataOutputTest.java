...
public final class ByteBuffersDataOutputTest extends RandomizedTest {
  ...
  public void testSanity() {
    ...
    assertNotEquals(0, o.ramBytesUsed());
  ...
  private static List<ThrowingBiFunction<DataOutput, Random, ThrowingConsumer<DataInput>>> GENERATORS;
  static {
  ...
  static List<ThrowingConsumer<DataInput>> addRandomData(DataOutput dst, Random rnd, int max) throws IOException {
    try {
      List<ThrowingConsumer<DataInput>> reply = new ArrayList<>();
      for (int i = 0; i < max; i++) {
